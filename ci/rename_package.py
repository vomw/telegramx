                    ...
                    for item in os.listdir(temp_dir):
                        ...
                        if os.path.exists(d) and os.path.isdir(d) and os.path.isdir(s):
                            ...
                            for sub_item in os.listdir(s):
                                shutil.move(
                                    os.path.join(s, sub_item), os.path.join(d, sub_item)
                                )
                            os.rmdir(s)
                        else:
                            shutil.move(s, d)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python rename_package.py <old_packages_comma_separated> <new_package>"
        )
        sys.exit(1)
